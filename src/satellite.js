...
let options = {}; // was const
let current; // allow reassignment
// Removed getCurrentSatelliteData() since it's undefined
      ...
      return new Promise((resolve, reject) => {
        request(utils.image_options(temp[property[0]]), (error, response, body) => {
          ...
          for (let i = 0; i < tbody.find("tr").length; i++) {
            const firstCell = tbody.find("tr").eq(i).find("td").eq(0).text();
            if (firstCell === "离开地影") {
